...
// Fork patch: upstream's "fail closed" gate (PR #5429) throws unconditionally,
// breaking every plugin that resolves secrets through ctx.secrets.resolve
// (paperclip-plugin-telegram, paperclip-plugin-slack, etc.). The cited
// "company-scoped plugin config" feature still hasn't landed (as of v2026.609).
// We restore the pre-#5429 working implementation here: look up the secret by
// UUID, fetch latest version material, delegate to the configured
// SecretProviderModule for resolution. Revert this patch once upstream lands
// the company-scoped path.
export function createPluginSecretsHandler(
  ...
  return {
    async resolve(params: PluginSecretsResolveParams): Promise<string> {
      ...
      if (!cachedAllowedRefs || now > cachedAllowedRefsExpiry) {
        const [configRow, plugin] = await Promise.all([
          db
            ...
            .then((rows: Array<{ configJson: unknown }>) => rows[0] ?? null),
      ...
      // 2. Look up the secret record
      const secret = await db
        .select()
        .from(companySecrets)
        .where(eq(companySecrets.id, trimmedRef))
        .then((rows: Array<{ id: string; provider: string; externalRef: unknown; latestVersion: number }>) => rows[0] ?? null);
      ...
      const versionRow = await db
        ...
        .then((rows: Array<{ material: Record<string, unknown> }>) => rows[0] ?? null);
      ...
      const resolved = await provider.resolveVersion({
        ...
        externalRef: secret.externalRef as string | null,
      });
